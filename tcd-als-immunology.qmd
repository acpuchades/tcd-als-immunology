...
imm_cytokines_l |>
  ...
  ggplot(aes(timepoint, il21, group=sample_id, color=KM)) +
  geom_line() +
  coord_cartesian(ylim=c(0, 17)) +
  labs(title = "Raw dataset", x = NULL, y = "CD8 IL-21")
```

Despite the apparent effect of higher CD8/IL-21 expression also at T1, **none of the patients in cluster 2 seem to have IL-21 levels available at T1, so we can't really draw any conclusions based on its longitudinal change**.
...
As we saw previously with CD8-related cluster 2, **the effect of increased IL-21 CD8 response is also associated with worse survival, and this effect is more pronounced in patients with an earlier age at onset**.
...
Let's try to reproduce the same analyses on CD8-associated IL-21 response using the unimputed dataset. This will allow us to assess the robustness of our findings and see if the imputation process has introduced any biases.
