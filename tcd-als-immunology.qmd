...
author:
  ...
  affiliation: "Institut d'Investigació Biomèdica de Bellvitge (IDIBELL), Barcelona, Spain"
...
imm_cyto_cd8.tsne.in |>
  ...
  reframe(
    cor.test(age, value, method = "spearman") |> tidy(),
    .by = "marker"
  )
```
...
PC3 seems to be the only primary component statistically associated with age, and it is negatively correlated with it.

Now let's take a look at each of the markers individually:
...
imm_cyto_cd8.tsne.in |>
  ...
  group_by(marker) |>
  reframe(cor.test(value, age) |> tidy()) |>
  arrange(p.value)
```
...
Despite the apparent negative correlation between PC3 and age, the only individual marker statistically associated with age seems to be IFN-gamma, and the correlation is negative.
...
```{r}
imm_cyto_cd8_age.cph <- coxph(
  Surv(disease_duration, vital_status) ~ KM + age_at_onset,
  data = imm_cyto_cd8.surv
)

summary(imm_cyto_cd8_age.cph)
```

```{r}
imm_cyto_cd8_age_i.cph <- coxph(
  Surv(disease_duration, vital_status) ~ KM * age_at_onset,
  data = imm_cyto_cd8.surv
)

summary(imm_cyto_cd8_age_i.cph)
```

Finally, this effect is also not fully explained by differences in age at onset, but there seems to be an interaction where older patients in cluster 2 have a slightly better survival compared to younger individuals within that cluster.
...
The same analysis by discretizing on the median IL-21 shows a similar trend in the unimputed dataset, with results not reaching statistical significance. This might be due to decreased power from the smaller sample size due to listwise deletion.

However, the Cox regression analysis on the unimputed dataset still shows a highly significant survival effect for IL-21-based CD8 response that is not fully explained by age at onset.
